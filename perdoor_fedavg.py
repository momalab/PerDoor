...
logging.basicConfig(filename='fl_training_log_fedavg.log', level=logging.INFO, format='%(message)s')
...
for r in range(1, utils.num_rounds):
    ...
    if not backdoor_inserted:
        ...
        if not adversary_selected:
            ...
            utils.fedavg(global_model, client_models, r, dataLoader.ni, dataLoader.ns)
            ...
            if len(attacker_selected_rounds) != args.analysis_window:
                ...
                global_original_model = torch.load(utils.model_path+f"round_{r-1}_global.pth")
                local_backdoor_model_dict = local_backdoor_model.state_dict()
                for k in local_backdoor_model_dict.keys():
                    local_backdoor_model_dict[k] = torch.add((dataLoader.ns/dataLoader.ni[utils.num_selected - 1])*(local_backdoor_model.state_dict()[k].float() - global_original_model.state_dict()[k].float()), global_original_model.state_dict()[k].float())
